...
- **編輯佈局模式**(選單「編輯佈局」按鈕):進入後遊戲模擬(訂單/顧客/計時)是暫停的,不會有顧客。用滑鼠/手指直接拖拉場景裡的物件調整位置(物件之間不會互相重疊,撞到會卡在邊緣),用 －/＋ 一次調整「全部物件」的大小,也可以用面板上的按鈕新增物件。編輯結果會自動存在這台裝置的瀏覽器裡(`localStorage`,重新整理/下次進來都會沿用),但**不會同步到別的裝置**;要讓兩支手機都拿到同一份佈局,還是要按「複製佈局」把結果複製起來貼給開發者,寫進程式碼正式部署。想丟掉目前存的、換回程式碼內建的預設佈局,按「還原成預設佈局」即可
